<?php
    class DashboardModel extends Mysql{
        ...
        public function selectCustomersYear(int $year){
            $arrInfo = array();
            $months = months();
            $total =0;
            for ($i=1; $i <=12 ; $i++) { 
                ...
                $sql = "SELECT $year as year, 
                        $i as month, 
                        count(idperson) as total 
                        FROM person
                        WHERE MONTH(date) = $i AND YEAR(date) = $year AND roleid = 2
                        GROUP BY MONTH(date)";
                $request = $this->select($sql);
                $arrData['month'] = $months[$i-1];
                if(empty($request)){
        ...
        public function selectCustomersMonth(int $year, int $month){
            $totalPerMonth = 0;
            $arrInfo = array();
            $days = cal_days_in_month(CAL_GREGORIAN,$month,$year);
            $day = 1;
            for ($i=0; $i < $days ; $i++) { 
                $date_create = date_create($year."-".$month."-".$day);
                $date_format = date_format($date_create,"Y-m-d");
                $sql ="SELECT DAY(date) AS day, COUNT(idperson) AS total FROM person WHERE DATE(date) = '$date_format' AND roleid = 2";
                $request = $this->select($sql);
                $request['day'] = $day;
                $request['total'] = $request['total'] =="" ? 0 : $request['total'];
                $totalPerMonth+=$request['total'];
