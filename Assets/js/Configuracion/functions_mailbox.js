...
if(document.querySelector("#correo")){
    ...
    formEmail.addEventListener("submit",function(e){
        ...
        request(base_url+"/Configuracion/Administracion/sendEmail",formData,"post").then(function(objData){
...
if(document.querySelector("#mensaje") && document.querySelector("#formReply")){
    ...
    formReply.addEventListener("submit",function(e){
        ...
        btn.innerHTML=`<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> `;
        btn.setAttribute("disabled","");
        request(base_url+"/Configuracion/Administracion/setReply",formData,"post").then(function(objData){
            btn.innerHTML=`<i class="fas fa-paper-plane"></i> Responder`;
            btn.removeAttribute("disabled");
            if(objData.status){
                window.location.reload();
...
function delMail(id,option){
    Swal.fire({
        ...
        if(result.isConfirmed){
            ...
            request(base_url+"/Configuracion/Administracion/delMail",formData,"post").then(function(objData){
                if(objData.status){
                    window.location.reload();
                }else{
                    Swal.fire("Error",objData.msg,"error");
