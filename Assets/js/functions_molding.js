...
searchPanel.addEventListener('input',function() {
    request(base_url+"/marqueteria/searchm/"+searchPanel.value,"","get").then(function(objData){
        if(objData.status){
...
sortPanel.addEventListener("change",function(){
    request(base_url+"/marqueteria/sortm/"+sortPanel.value,"","get").then(function(objData){
        if(objData.status){
            element.innerHTML = objData.data;
        }else{
            element.innerHTML = objData.data;
