...
searchPanel.addEventListener('input',function() {
    request(base_url+"/marqueteria/searchc/"+searchPanel.value,"","get").then(function(objData){
        if(objData.status){
...
sortPanel.addEventListener("change",function(){
    request(base_url+"/marqueteria/sortc/"+sortPanel.value,"","get").then(function(objData){
        if(objData.status){
            element.innerHTML = objData.data;
        }else{
            element.innerHTML = objData.data;
