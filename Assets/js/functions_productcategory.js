...
searchPanel.addEventListener('input',function() {
    request(base_url+"/inventario/searchc/"+searchPanel.value,"","get").then(function(objData){
        if(objData.status){
...
sortPanel.addEventListener("change",function(){
    request(base_url+"/inventario/sortc/"+sortPanel.value,"","get").then(function(objData){
        if(objData.status){
            element.innerHTML = objData.data;
        }else{
            element.innerHTML = objData.data;
